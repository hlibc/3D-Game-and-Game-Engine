...
bool md2::load(char* filename)
{
	...
	ret = fread(&header, sizeof(md2_header), 1, file);
	...
	ret = fread(frames, header.frameSize*header.numFrames, 1, file);
	...
	ret = fread(glCommands, header.numGlCommands*sizeof(int), 1, file);
...
{ 
	...
	fatal(name1); // suppress compiler warning CM Graff */
